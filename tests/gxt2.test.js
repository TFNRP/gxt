...
const data = fs.readFileSync(path.join(__dirname, 'assets', 'five.gxt2'));
...
describe('GXT2', () => {
  ...
  test('has expected first entry (string->hash)', () => {
    expect(gxt2.has('LCPD_BUMF_B2')).toBe(true);
    expect(gxt2.get('LCPD_BUMF_B2')).toBe('Painted Bumper w/ Protectors MkI');
  });
  ...
  test('entry can be changed as expected', () => {
    ...
    expect(gxt2.buffer.length).toBe(previousLength - entryDiff);
  });
});